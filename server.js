...
app.get("/api/ideas", async (req, res) => {
    ...
    // 키가 없을 때도 프론트 기능 확인 가능하도록 폴백 제공
    if (!clientId || !clientSecret) {
        return res.json({
            ...
            message: "NAVER API 키 미설정 상태입니다. .env 설정 후 실데이터 조회가 가능합니다.",
        });
    }
    ...
    try {
        ...
        if (!menuResp.ok || !dateResp.ok || dateLocalResps.some((r) => !r.ok)) {
            ...
            const dateLocalErrs = await Promise.all(
                dateLocalResps.map(async (r) => (r.ok ? "" : await r.text()))
            );
            return res.status(502).json({
                error: "네이버 API 호출 실패",
                details: { menuErr, dateErr, dateLocalErrs },
            });
        }
        ...
        const menusFromTitle = pickUniqueKeywords(
            (menuData.items || []).map((item) => item.title),
            []
        ).filter((v) => !looksLikePlaceName(v));

        const menusFromCategory = (menuData.items || [])
            .map((item) => toMenuFromCategory(item.category || ""))
            .filter(Boolean);

        const menus = pickUniqueKeywords(
            [...menusFromTitle, ...menusFromCategory],
            ["한식 메뉴", "중식 메뉴", "일식 메뉴", "양식 메뉴", "분식 메뉴"]
        );

        const courses = pickUniqueKeywords(
            (dateData.items || []).flatMap((item) => [item.title, item.description]),
            ["영화 보기", "산책", "카페 가기", "전시회 관람", "드라이브"]
        ).filter((v) => !looksLikePlaceName(v));
        ...
        let datePlaceRaw = mergedDateLocalItems.filter(
            (item) => !isFoodCategory(item.category || "") && isDateFriendlyCategory(item.category || "", item.title || "")
        );

        // 1차가 너무 엄격하면 음식 카테고리만 제외하는 완화 필터
        if (datePlaceRaw.length < 5) {
            datePlaceRaw = mergedDateLocalItems.filter((item) => !isFoodCategory(item.category || ""));
        }

        // 그래도 부족하면 이름 기반으로 데이트 친화 키워드 재선별
